				...
				<div :class="aClassVal" @click='Func(model,$event)' @contextmenu.prevent='cxtmenufunc'>
					...
					<div class="dropdown" @mouseenter="operate($event)" @mouseleave="leaveNode($event)" >
						...
						<i class="glyphicon glyphicon-cog dropdown-toggle" @click="dropDownList"  title="操作"
						   ...
						   v-show="simulate.indexOf('Department/position_update')>-1||simulate.indexOf('Department/position_insert')>-1
        ...
        computed:{
            ...
            rootClass(){
                ...
                if(this.root==='0'){
                    ...
                    strRootClass =  (this.num===0 && this.model.position.length===0) ?
                    ...
                    strRootClass =  this.nodes>1 && this.model.position.length>0 && this.nodes!==this.num+1 ?
                        "center_" : (this.num === 0 && this.nodes>1) || (this.nodes!==this.num+1) ?
                            "center_docu" : this.nodes === 1&&this.num!==0 || (this.nodes===this.num+1 && this.model.position.length>0) ?
                                "bottom_" : "bottom_docu";
                }
            ...
            prefixClass(){
                ...
                if(this.model.position.length===0 && this.rootClass.indexOf("docu")===-1){
                    returnChar = 'docu'
                }
            ...
            ulClassVal(){
                return this.isChildren && this.model.position.length>0 ?"level"+this.num+' line':"level"+this.num;
            },
        },
        methods:{
            /*// 登陆状态/权限列表
...
<style scoped>
	...
	.ztree li ul{ margin:0; padding:0 0 0 40px}
